    ...
    return `${itemNameId}::${specificationId}`;
		  ...
		  const [loadingFirms, setLoadingFirms] = useState(true);
			  ...
			  const [loadingProjects, setLoadingProjects] = useState(true);
				  const [issueType, setIssueType] = useState<'Sales' | 'Project' | 'Internal Use'>('Sales');
		  ...
		  const [items, setItems] = useState<ItemDraft[]>([{ itemId: '', itemNameId: '', item: '', quantity: '', specification: '', specs: {} }]);
    ...
    createItemCategory({ name, createdBy: 'system' })
      ...
      .finally(() => setCreateCategoryInlineBusy(false));
							  ...
							  const canSubmit = useMemo(() => {
							    ...
							    if (issueType === 'Internal Use' && !department.trim()) return false;
        ...
        if (!createItemNameInlineCategoryId) {
		  ...
		  const closeCreateValue = () => {
		    ...
		    createSpecificationValue({ specificationId: specId, value: v, createdBy: 'system' })
	          ...
	          <label className="space-y-1">
	            <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Issue By</div>
	            <SearchableSelect
	            ...
	            />
		          ...
		          <label className="space-y-1">
		            <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Issue Date</div>
			        ...
			        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
			          <label className="space-y-1">
			            <div className="text-[10px] font-bold text-on-surface-variant uppercase tracking-wider">Issue Type</div>
			            <SearchableSelect
			              value={issueType}
			              options={[
			              ...
			              onChange={(v) => setIssueType(v === 'Project' || v === 'Internal Use' ? v : 'Sales')}
				          ...
				          <div>
			                ...
			                />
								          ...
								          <div className="border-t border-outline-variant bg-surface-container-lowest px-2 py-2 flex items-center justify-between gap-3">
								            <div className="flex items-center gap-3">
								                ...
								                className="btn-primary"
									                onClick={() => setItems((prev) => [...prev, { itemId: '', itemNameId: '', item: '', quantity: '', specification: '', specs: {} }])}
							              >
						                + Add Item
						              </button>
					            </div>
                              ...
                              const itemNameId = String(it.itemNameId ?? '').trim();
						                      ...
						                      return { itemId, itemNameId, specs: it.specs ?? {}, item: itemName, quantity: quantityNumber, specification };
						                    })
				                  ...
				                  if (issueType === 'Internal Use' && !department.trim()) {
				                    setError('Please select a Department for Internal Use issues.');
				                    return;
				                  }
