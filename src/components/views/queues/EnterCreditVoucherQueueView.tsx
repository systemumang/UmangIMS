      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <label className="space-y-1 md:col-span-2">
            <div className={labelClass}>Voucher No (optional)</div>
            <input className={inputClass} value={voucherNumber} onChange={(e) => setVoucherNumber(e.target.value)} placeholder="CV-..." />
          </label>
        </div>
                  ...
                  <tr key={ln.itemId} className="hover:bg-surface-container-low/50 transition-colors">
                    ...
                    <td className="px-3 py-2 border border-outline-variant">
                      <input
                        className={cn(inputClass, 'h-9 text-right')}
                        value={ln.rate}
                        onChange={(e) => setLines((prev) => prev.map((p, i) => (i === idx ? { ...p, rate: e.target.value } : p)))}
                        placeholder="0"
                      />
                    </td>
                  </tr>
