  ...
  for (const [lineIndex, row] of items.entries()) {
    ...
    await pool.query(
      ...
      [
        ...
        totalAmount,
        'system',
        areaUnit ? finiteNumberOrNull(round2(dimLength)) : null,
        areaUnit ? finiteNumberOrNull(round2(dimBreadth)) : null,
        areaUnit ? finiteNumberOrNull(Math.trunc(dimPcs)) : null,
...
app.get('/api/operations/pos/:id', async (req, res) => {
  try {
    ...
    const qtyByPoItemId = new Map();
    for (const r of Array.isArray(qtyRows) ? qtyRows : []) {
      qtyByPoItemId.set(String(r.poItemId ?? ''), {
        grnQty: Number(r.grnQty ?? 0),
...
app.post('/api/requests', async (req, res) => {
    ...
    const remarks = JSON.stringify({ ...(department ? { department } : {}), ...(remarksInput ? { remarks: remarksInput } : {}) });
      ...
      await pool.query(
            ...
            areaUnit ? finiteNumberOrNull(round2(dimLength)) : null,
            areaUnit ? finiteNumberOrNull(round2(dimBreadth)) : null,
            areaUnit ? finiteNumberOrNull(Math.trunc(Number(dimPcs))) : null,
            areaUnit ? dimUnit : null,
          ]
	      );
	    }
      ...
      if (itemIds.length) {
        ...
        if (areaUnit) {
	      ...
	      outItems.push({
	        ...
	        taxAmount,
	        totalAmount,
          dimLength: areaUnit ? finiteNumberOrNull(round2(dimLength)) : null,
          dimBreadth: areaUnit ? finiteNumberOrNull(round2(dimBreadth)) : null,
          dimPcs: areaUnit ? finiteNumberOrNull(Math.trunc(Number(dimPcs))) : null,
      ...
      if (String(poRow.status ?? '').trim().toLowerCase() === 'draft') {
        ...
        if (isArea) {
	      ...
	      await pool.query(
            ...
            'system',
            updatedBy || null,
            isArea ? finiteNumberOrNull(round2(dimLength)) : null,
            isArea ? finiteNumberOrNull(round2(dimBreadth)) : null,
	            isArea ? finiteNumberOrNull(Math.trunc(Number(dimPcs))) : null,
          ...
          recvDimLength: isArea ? finiteNumberOrNull(round2(dimLength)) : null,
          recvDimBreadth: isArea ? finiteNumberOrNull(round2(dimBreadth)) : null,
          recvDimPcs: isArea ? finiteNumberOrNull(Math.trunc(Number(dimPcs))) : null,
    ...
    const normalizedItems = items
      .map((it) => {
        ...
        return {
          ...
          dimLength: isArea ? finiteNumberOrNull(round2(dimLength)) : null,
          dimBreadth: isArea ? finiteNumberOrNull(round2(dimBreadth)) : null,
          dimPcs: isArea ? finiteNumberOrNull(Math.trunc(Number(dimPcs))) : null,
